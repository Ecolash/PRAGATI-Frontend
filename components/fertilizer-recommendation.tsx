...
import { Loader2, FlaskConical, BarChart3, TrendingUp, Sprout, CheckCircle2, Zap, Target } from "lucide-react"
...
export function FertilizerRecommendation() {
  ...
  const getFertilizerRecommendation = async (): Promise<ApiResponse> => {
    try {
      ...
      console.log("getFertilizerRecommendation - Response type:", typeof response)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to get fertilizer recommendations. Please try again.")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-md border-b border-yellow-200 px-4 py-3">
        <div className="flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 mt-1">Smart recommendations for optimal crop growth</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4 auto-rows-min">
          <Card className="md:col-span-3 bg-white/90 backdrop-blur-sm border border-yellow-200 shadow-lg">
            <CardHeader className="pb-4">
              ...
              <CardDescription className="text-gray-600">Climate conditions for optimal growth</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {(["temperature", "humidity", "moisture"] as const).map((param) => {
                  ...
                  <div key={param} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-sm font-medium flex items-center gap-2">
                        ...
                        <span className="capitalize text-gray-700">{param}</span>
                      </Label>
                      <Badge variant="secondary" className="bg-yellow-100  border-yellow-200">
                        {formData[param]}
                        {config.unit}
                      </Badge>
                    </div>
              ...
              <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <h4 className="text-sm font-medium  mb-2">💡 Environmental Tips</h4>
          ...
          <Card className="md:col-span-3 bg-white/90 backdrop-blur-sm border border-yellow-200 shadow-lg">
            <CardHeader className="pb-4">
              ...
              <CardDescription className="text-gray-600">NPK levels in mg/kg for soil analysis</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {(["nitrogen", "potassium", "phosphorous"] as const).map((param) => {
                const config = parameterConfig[param]
                return (
                  <div key={param} className="space-y-2">
                    <div className="flex items-center justify-between">
          ...
          <Card className="md:col-span-2 lg:col-span-2 xl:col-span-3 bg-white/90 backdrop-blur-sm border border-yellow-200 shadow-lg">
            <CardHeader className="pb-4">
              ...
              <CardDescription className="text-gray-600">Choose your farming context and conditions</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleDropdownChange("soil_type", value)}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleDropdownChange("crop_type", value)}
                  >
                    <SelectTrigger className="bg-white border-yellow-200 focus:border-yellow-400 focus:ring-yellow-400">
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {formData.soil_type === "Sandy" && "Good drainage, low nutrients"}
                      {formData.soil_type === "Loamy" && "Balanced, ideal for most crops"}
                      {formData.soil_type === "Black" && "Rich in nutrients, cotton-friendly"}
                      {formData.soil_type === "Red" && "Iron-rich, good for cereals"}
                      {formData.soil_type === "Clayey" && "High water retention, dense"}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {formData.crop_type === "Paddy" && "High water, moderate NPK"}
                      {formData.crop_type === "Wheat" && "Nitrogen-rich, cool climate"}
                      {formData.crop_type === "Cotton" && "Potassium-heavy, warm climate"}
                      {formData.crop_type === "Maize" && "Balanced NPK, good drainage"}
                      {formData.crop_type === "Sugarcane" && "High nitrogen, long season"}
                      {!["Paddy", "Wheat", "Cotton", "Maize", "Sugarcane"].includes(formData.crop_type) &&
              ...
              <div className="p-3 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-lg border border-yellow-200">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="text-gray-600">
                    Temperature: <span className="font-medium">{formData.temperature}°C</span>
                  </div>
                  <div className="text-gray-600">
                    Humidity: <span className="font-medium">{formData.humidity}%</span>
                  </div>
                  <div className="text-gray-600">
                    N: <span className="font-medium">{formData.nitrogen} mg/kg</span>
                  </div>
                  <div className="text-gray-600">
                    P: <span className="font-medium">{formData.phosphorous} mg/kg</span>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-yellow-800 text-sm">All parameters configured and validated</p>
                </div>
              </div>
              ...
              <div className="mt-4 p-3 bg-white/20 rounded-lg">
                ...
                <p className="text-xs text-yellow-800 leading-relaxed">
                  Your configuration is complete! Our AI will analyze {formData.crop_type.toLowerCase()} growth
                  requirements for {formData.soil_type.toLowerCase()} soil conditions and recommend the optimal
                  fertilizer blend.
                </p>
              </div>
            </CardContent>
              ...
              <Card className="md:col-span-2 lg:col-span-2 xl:col-span-3 bg-gradient-to-br from-yellow-400 via-amber-300 to-yellow-500 text-yellow-900 border-0 shadow-xl">
                <CardContent className="p-6 text-center">
                  <div className="text-7xl mb-4">
                    {fertilizerMetadata[recommendations.recommended_fertilizer || ""]?.emoji || "🧪"}
                  </div>
                  <h2 className="text-2xl font-bold mb-3">{recommendations.recommended_fertilizer || "Unknown"}</h2>
                  <Badge className="bg-white/30 text-yellow-900 border-white/40 text-lg px-6 py-2 mb-4 shadow-lg">
                    {Math.round((recommendations.confidence || 0) * 10000) / 100}% Match
                  </Badge>
                  <p className="text-sm mb-3 text-yellow-800 leading-relaxed">
                    {fertilizerMetadata[recommendations.recommended_fertilizer || ""]?.description}
                  </p>
                  <p className=" text-xs italic">
                    {fertilizerMetadata[recommendations.recommended_fertilizer || ""]?.application}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="md:col-span-2 lg:col-span-4 xl:col-span-6 bg-white/90 backdrop-blur-sm border border-yellow-200 shadow-lg">
                <CardHeader className="pb-4">
                  ...
                  <CardDescription className="text-gray-600">
                    Top fertilizer suitability scores for your specific conditions
                  </CardDescription>
                </CardHeader>
